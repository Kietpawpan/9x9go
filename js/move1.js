			...
			if(board[i][j] !== EMPTY && board[i][j] !== player) {
				if(getLiberties(board, i, j) === 0) {
					...
					for(var k = 0; k < chain.length; k++) {
						...
						var pY = chain[k][1];
						board[pX][pY] = EMPTY;
	//Montri create move number
	var move = document.getElementById('move').innerHTML.replace('Move','');
	var bscore =  document.getElementById('bscore').innerHTML;
	var wscore =  document.getElementById('wscore').innerHTML;
	var newBs ='';
	var newWs ='';
	if(move % 2==0){newBs = Number(bscore) + 1;
	document.getElementById('bscore').innerHTML = newBs;}
	else{newWs = Number(wscore) + 1;
	document.getElementById('wscore').innerHTML = newWs;}
...
//Montri create move number
	var move =  document.getElementById('move').innerHTML.replace('Move','');
	let moveNo = Number(move) + 1;
	var score = document.getElementById('move').innerHTML = 'Move ' + moveNo;
...
function w(ctx, x , y){
	ctx.fillStyle = '#fff';
	ctx.beginPath();
	ctx.arc(rootX + boxSize * x, rootY + boxSize * y, 24.5, 0, 2 * Math.PI);
	ctx.fill();
...
function fillNumB(ctx, x, y, z){
	ctx.fillStyle = '#fff';
	ctx.beginPath();
	ctx.font='18px Arial';
	ctx.fillText(z, rootX + boxSize * x - 5, rootY + boxSize * y + 5);
}
